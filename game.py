...
def shift_up(grid):
    #เลื่อนและรวมตัวเลขขึ้น
    new_grid = [[0]*4 for _ in range(4)]  # สร้างตารางใหม่ 4x4 ที่เต็มไปด้วยศูนย์

    # ทำทีละคอลัมน์ (แนวตั้ง)
    for c in range(4):
        # ดึงค่าจากคอลัมน์นั้นทั้งหมด
        col = [grid[r][c] for r in range(4) if grid[r][c] != 0]

        # รวมเลขที่เหมือนกัน (เหมือนกับ shift_left)
        i = 0
        while i < len(col) - 1:
            if col[i] == col[i + 1]:
                col[i] *= 2
        ...
        # เติม 0 ด้านล่างให้ครบ 4 ตัว
        while len(col) < 4:
            col.append(0)

        # ใส่ค่ากลับลงใน new_grid
        for r in range(4):
            new_grid[r][c] = col[r]
...
def shift_down(grid):
    #เลื่อนและรวมตัวเลขลง
    new_grid = [[0]*4 for _ in range(4)]

    for c in range(4):
        # ดึงค่าจากคอลัมน์นั้นทั้งหมด (จากล่างขึ้นบน)
        col = [grid[r][c] for r in range(3, -1, -1) if grid[r][c] != 0]
        ...
        # เติม 0 ด้านบนให้ครบ 4 ตัว
        while len(col) < 4:
            col.append(0)

        # ใส่ค่ากลับลงใน new_grid (จากล่างขึ้นบน)
        for r in range(4):
